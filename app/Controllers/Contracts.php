...
class Contracts extends Security_Controller {
    ...
    function save() {
        ...
        if ($contract_id) {
            ...
            $web3Handler = new \App\Libraries\Web3Handler();
            $contract_data['id'] = $contract_id;
            $web3Handler->createContractOnBlockchain($contract_data);
        ...
        if ($contract_id && $status) {
            ...
            if ($this->login_user->user_type == "client") {
                ...
                if ($contract_info->status == "sent" && ($status == "accepted" || $status == "declined")) {
                    ...
                    $web3Handler = new \App\Libraries\Web3Handler();
                    $web3Handler->updateContractStatus($contract_id, $status);
